...
Intention of the library is to generate a valid SQL Sequel. Is not an ORM and is not intended to use it for validating
query input
...
As viewed in [WHERE](#filters) clause, `Sequel.having` can be repeated multiple times using `Criterion` class logic
